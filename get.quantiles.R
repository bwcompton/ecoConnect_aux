'get.quantiles' <- function(sourcepath = 'c:/gis/lcc/ecoConnect_final/', postfix) {
   ...
   f <- list.files('c:/gis/lcc/ecoConnect_final/', paste0('ec_samples_', postfix, '.*\\.RDS$'))
   for(i in 1:length(files)) {
      x <- readRDS(paste0(sourcepath, f[i]))
      ...
      if(i == 1) {
       ...
       realized.acres <- acres  ###########  realized.acres <- x$realized.acres
         ...
         z <- rbind(z, x$samples)                           # is this merely a bad idea or a terrible idea? Alternatively, we could read each file to see how big
         #                                                  # it is, or write a directory of files and sizes in sample.for.quantiles. I'll see how bad this line is.
